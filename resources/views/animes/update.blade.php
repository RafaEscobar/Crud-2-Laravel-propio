...
@section('title', 'Actualización de registro')

@section('content')
<div class="w-full flex justify-center">
    <div class="w-6/12">
        <h1 class="text-3xl mb-8">Actualizar registro:</h1>
            <form action="{{route('animes.submit', $id)}}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="grid sm:grid-cols-1 md:gap-6">
                  <div class="relative z-0 w-full mb-6 group">
                      <input type="text" name="nombre" id="nombre" class="block py-2.5 px-0 w-full bg-transparent border-0 border-b-2 appearance-none dark:text-black dark:border-gray-600  focus:outline-none focus:border-blue-600 peer" value="{{$id->nombre}}" required />
                  ...
                  <div class="relative z-0 w-full mb-6 group">
                      <input type="number" name="capitulos" id="capitulos" class="block py-2.5 px-0 w-full bg-transparent border-0 border-b-2 appearance-none dark:text-black dark:border-gray-600  focus:outline-none focus:border-blue-600 peer" value="{{$id->capitulos}}" required />
                ...
                <div id="element">
                    <div class="w-full mb-6">
                        <img id="imgSelect" alt="img" class="w-72 h-48 m-auto" src="/imgs/{{$id->file_path}}">
                    </div>
                </div>
